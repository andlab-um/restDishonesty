...
# EMRVR model
def RVR(X_train, y_train, X_test, y_test, feature_index):
    """Building an EMRVR model and get predictions
    Returns
    -------
    dict
    ...
    model = EMRVR(kernel="linear", bias_used=False)
...
# prediction pipeline
def prediction(data1_X, data1_y, data2_X, data2_y, cv="loocv", feature_index=None, validation="model", seed=9873):
    """Get prediction
    ...
    validation : str, optional
        prediction approach, can be "model" or "feature", by default "model".
        "model" means test the data on the same model,
        "features" means test the data on the same feature_index, but not the same model
    Returns
    -------
    ...
    if validation == "model":
        result = [RVR(data1_X, data1_y, data2_X, data2_y, feature_index)]
    # train and test on the same dataset, here data1 is the same as data2
    elif validation == "feature":
        # define kfold
        if cv == "loocv":
        ...
        result = Parallel(n_jobs=32)(delayed(RVR)(data1_X[train_index], data1_y[train_index], data2_X[test_index],
...
def permutation(d1_X, d1_y, d2_X, d2_y, true_coef, feature_index=None, validation="model", permutation_n=1000):
    ...
    validation : str, optional
        Training methods, by default "model", see code for more information
